...
#
# v1.1 12 Sep 16 - fixed a bug which mapped atomic indices and forces wrongly 
    	...
    	for every_ion in xrange(0,ions):	
			...
			for y in xrange(0,ny):					
				...
				for x in xrange(0,nx):				     
                                     ...
                                     indices_array.append([int(every_atom+1), int((modnx+1+nx*modny+modnz*ny*nx + every_ion*nz*ny*nx))])
                                 
    # print indices_array ,'\n'  #indices seem correct, the bug should be in the mapping between idices and forces                           
   		   ...
   		   for key in keylist: 
    			    ...
    			    i+=1       
     			    # here [i-1][0] and [i-1][1] define the direction of the forces
                            ion_couple = '%4s' %indices_array[i-1][0] + '%4s' %indices_array[i-1][1]
